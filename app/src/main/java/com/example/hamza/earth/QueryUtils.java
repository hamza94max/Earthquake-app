...
public final class QueryUtils {
    ...
    public static ArrayList<Model> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray earth  = reader.getJSONArray("features");
            for (int i = 0; i < earth.length(); i++) {
                JSONObject features = earth.getJSONObject(i);
                JSONObject properties =features.getJSONObject("properties");
                double str = properties.getDouble("mag");
                String location = properties.getString("place");
                long time = properties.getLong("time");
                Date dateObject = new Date(time);
                SimpleDateFormat dateFormatter = new SimpleDateFormat("MMM DD, yyyy");
                String dateToDisplay = dateFormatter.format(dateObject);
                Model model=new Model( str,location,dateToDisplay);
                String url = properties.getString("url");
                Model mm=new Model(str,location,dateToDisplay,url);
                earthquakes.add(mm);
            }
